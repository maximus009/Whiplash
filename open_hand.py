...
import os
print "0. EXIT\n1. IDLE\n2. FIFA-13\n3. CHROME\n4. NOTEPAD\n6. MOVIES"
pgm_list = ["exit","idle", "fifa", "chrome", "notepad","exit", "movies"]
c = 0
...
while True:
    ...
    while I<100:
        cv2.putText(thresh, "LOADING... in "+str(99-I), (10,30), cv2.FONT_HERSHEY_SIMPLEX, 1, 255)
        cv2.putText(thresh, "Fit the circle on your palm.", (10,50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, 255)
        ...
        cv2.circle(thresh, (180,180),50,127)
        for j in range(I):
...
##        c = input("What Do You Wish To Open:")
        if fings==c:
            chk+=1
        else:
            chk = 0
        c=fings
        if chk == 25:
            cv2.putText(thresh,"Identified " + pgm_list[c], (10,200),cv2.FONT_HERSHEY_SIMPLEX,0.4,125)
            cv2.putText(thresh,"Press ESC to open.",(10,220),cv2.FONT_HERSHEY_SIMPLEX,0.4,125)
            cv2.imshow("BINARY",thresh)
            if cv2.waitKey(0) == 27:
                cv2.destroyAllWindows()
                os.system(pgm_list[c])
                break
